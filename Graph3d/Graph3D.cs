...
#pragma warning restore CS8622 // Допустимость значений NULL для ссылочных типов в типе параметра не соответствует целевому объекту делегирования (возможно, из-за атрибутов допустимости значений NULL).
        ...
        private void NormalizeRanges(eNormalize e_Normalize)
        {
            ...
            switch (e_Normalize)
            {
                ...
                case eNormalize.MaintainXYZ:
                    ...
                    d_RangeZ = d_RangeXYZ;
                    break;
                case eNormalize.Separate:
                    double d_RangeXZ = (d_RangeX + d_RangeY + d_RangeZ) / 7;
                    d_RangeZ = d_RangeXZ;
                    break;
            }
        ...
        private void CreateCoordinateSystem(Graphics i_Graph)
        {
            ...
            for (int A = 0; A < 3; A++)
            {
                ...
                {
                    case eCoord.X: // Blue
                        i_Axis.mi_Points3D[0].md_X = mi_MinMax.md_MinX;
                        i_Axis.mi_Points3D[1].md_X = mi_MinMax.md_MaxX;
                        // ---------------------------
                        i_Axis.mi_Points3D[0].md_Y = mi_MinMax.md_MinY; // X axis at minimum Y position
                        i_Axis.mi_Points3D[1].md_Y = mi_MinMax.md_MinY; // X axis at minimum Y position

                        i_Axis.mi_Points3D[0].md_Z = mi_MinMax.md_MinZ;
                        i_Axis.mi_Points3D[1].md_Z = mi_MinMax.md_MinZ;
                        i_Axis.me_Line = eCoord.X;
                        i_Axis.me_Offset = eCoord.Y; // Hide zero label at X axis (X,X invalid)
                        i_Axis.md_Label = mi_MinMax.md_MinY;
                        
                        break;
                    case eCoord.Y: // Green
                        i_Axis.mi_Points3D[0].md_Y = mi_MinMax.md_MinY;
                        i_Axis.mi_Points3D[1].md_Y = mi_MinMax.md_MaxY;
                        // ---------------------------
                        i_Axis.mi_Points3D[0].md_X = mi_MinMax.md_MinX; // Y axis at minimum X position
                        i_Axis.mi_Points3D[1].md_X = mi_MinMax.md_MinX; // Y axis at minimum X position

                        i_Axis.mi_Points3D[0].md_Z = mi_MinMax.md_MinZ;
                        i_Axis.mi_Points3D[1].md_Z = mi_MinMax.md_MinZ;
                        i_Axis.me_Line = eCoord.Y;
                        i_Axis.me_Offset = eCoord.X; // Show zero label at Y axis
                        i_Axis.md_Label = mi_MinMax.md_MinX;
                        
                        break;
                    case eCoord.Z: // Red
                        i_Axis.mi_Points3D[0].md_Z = mi_MinMax.md_MinZ;
                        i_Axis.mi_Points3D[1].md_Z = mi_MinMax.md_MaxZ;
                        // ---------------------------
                        i_Axis.mi_Points3D[0].md_X = mi_MinMax.md_MinX; // Z axis start at minimum X position
                        i_Axis.mi_Points3D[1].md_X = mi_MinMax.md_MinX; // Z axis start at minimum X position
                        i_Axis.mi_Points3D[0].md_Y = mi_MinMax.md_MinY; // Z axis start at minimum Y position
                        i_Axis.mi_Points3D[1].md_Y = mi_MinMax.md_MinY; // Z axis start at minimum Y position
                        i_Axis.me_Line = eCoord.Y;
                        i_Axis.md_Label = mi_MinMax.md_MaxZ;
            ...
            if (me_Raster >= eRaster.Raster)
            {
                for (int A = 0; A < 3; A++) // iterate axis X,Y,Z
                {
                    ...
                    for (int D = 0; D < 2; D++) // iterate first, second
                    {
                        ...
                        {
                            double d_Offset = d_Interval * L + d_SecndStart;
